...
if($result!="验签失败"){
	if($serverid == 'sy01') {
		$index_url = $req_domain . "?uri=index.php&sid=$serverid&pid=$pid"; 
		$alipayto_url = $req_domain;
	} else if($serverid == 'sy02')  { 
		$index_url = $req_domain_2f . "?uri=index.php&sid=$serverid&pid=$pid";
		$alipayto_url = $req_domain_2f;
	} else if($serverid == 'sy03') {
		$index_url = $req_domain_3f . "?uri=index.php&sid=$serverid&pid=$pid";
		$alipayto_url = $req_domain_3f;
	} else if($serverid == 'sy04') {
		$index_url = $req_domain_4f . "?uri=index.php&sid=$serverid&pid=$pid";
		$alipayto_url = $req_domain_4f;
	} else if($serverid == 'sy05') {
		$index_url = $req_domain_5f . "?uri=index.php&sid=$serverid&pid=$pid";
		$alipayto_url = $req_domain_5f;
	} else if($serverid == 'sy06') {
		$index_url = $req_domain_6f . "?uri=index.php&sid=$serverid&pid=$pid";
		$alipayto_url = $req_domain_6f;
	}	
	$html = '';
	if(isset($result->payChannleResult->lastestPayChannel)) {
		if(!is_null($result->payChannleResult->lastestPayChannel)){
			...
			if(isset($result->payChannleResult->lastestPayChannel->cashierCode)) {
				...
				$html = "<h3>{$alipay_lang['recently_used']}</h3><a href=\"{$alipayto_url}?uri=alipayto.php&id=" . $lastCode . "&pid={$pid}&amt={$amt}&sid={$serverid}\">" + lastName + "</a><br>";
			}
		}
	}
	...
	{		
		...
		{
			$html = $html."<a href=\"{$alipayto_url}?uri=alipayto.php&id=".$secList[$j]->cashierCode."&pid={$pid}&amt={$amt}&sid={$serverid}\">".$secList[$j]->name."信用卡"."</a><br />";
		}
	}
	...
	if($topList[1]->cashierCode=="DEBITCARD")
	{
		...
		for($j=0; $j<count($secList); $j++)
		{
			$html = $html."<a href=\"{$alipayto_url}?uri=alipayto.php&id=".$secList[$j]->cashierCode."&pid={$pid}&amt={$amt}&sid={$serverid}\">".$secList[$j]->name."储蓄卡"."</a><br />";
		}	
    ...
    <div class="tips2">{$alipay_lang['curr_recharge']}<strong>{$amt}{$alipay_lang['yuan']}</strong>({$got_ingot}{$alipay_lang['gold_ingot']})</div>
      <div class="CZ">
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="chongzhi">
          ...
          <tr>
            <td><span class="zfbzf"><a href="{$alipayto_url}?uri=alipayto.php&pid={$pid}&amt={$amt}&sid={$serverid}">{$alipay_lang['alipay_account']}</a></span></td>
          </tr>
          <tr>
            <td><div class="qtzf">
				$html
