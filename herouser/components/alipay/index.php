...
if(!isset($_REQUEST['submit'])) {
	...
	$serverid = isset($_REQUEST["sid"]) ? $_REQUEST["sid"] : null;
	$server_name = array('sy01'=>$alipay_lang['server1_name'], 'sy02'=>$alipay_lang['server2_name'],'sy03'=>$alipay_lang['server3_name'], 'sy04'=>$alipay_lang['server4_name'], 'sy05'=>$alipay_lang['server5_name'], 'sy06'=>$alipay_lang['server6_name']);
	...
	if($_REQUEST['pname'] == 0) {
		...
		Header("Location: alipaychannel.php?amt=$amt&pid=$pid&sid=$sid");	
		exit;
	}
}
...
<div class="Wrap">
    ...
    <form action="<?php if($_GET['sid'] == 'sy01'){ echo $req_domain . '?uri=index.php';} else if($_GET['sid'] == 'sy02') {echo $req_domain_2f . '?uri=index.php';} else if($_GET['sid'] == 'sy03'){echo $req_domain_3f . '?uri=index.php';} else if($_GET['sid'] == 'sy04'){echo $req_domain_4f . '?uri=index.php';} else if($_GET['sid'] == 'sy05'){echo $req_domain_5f . '?uri=index.php';} else if($_GET['sid'] == 'sy06'){echo $req_domain_6f . '?uri=index.php';}?>" method="post">
    <div class="Main">
      <div class="CZ">
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="chongzhi">
          ...
          <tr>
              ...
              <tr>
                <td align="right"><span class="czleft"><a href="<?php if($_GET['sid'] == 'sy01'){ echo $req_domain . '?uri=index2.php&pid='.$pid.'&sid='.$serverid;} else if($_GET['sid'] == 'sy02') {echo $req_domain_2f . '?uri=index2.php&pid='.$pid.'&sid='.$serverid;} else if($_GET['sid'] == 'sy03') {echo $req_domain_3f . '?uri=index2.php&pid='.$pid.'&sid='.$serverid;} else if($_GET['sid'] == 'sy04') {echo $req_domain_4f . '?uri=index2.php&pid='.$pid.'&sid='.$serverid;} else if($_GET['sid'] == 'sy05') {echo $req_domain_5f . '?uri=index2.php&pid='.$pid.'&sid='.$serverid;} else if($_GET['sid'] == 'sy06') {echo $req_domain_6f . '?uri=index2.php&pid='.$pid.'&sid='.$serverid;} ?>"><?php echo $alipay_lang['other_amt'];?></a></span></td>
        ...
        <div class="fhbtn"><a href="http://q.jofgame.com/Pay_zfb.php"><!--<div class="fhbtn"><a href="<?php echo $merchant_url ?>">--><img src="Images/fh.jpg" width="50" height="28" /></a></div>
