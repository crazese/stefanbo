	...
	if($status == 'TRADE_FINISHED' || $status == 'TRADE_SUCCESS') {
		...
		if ($rows ['verify'] == 0) { // 检查是否已验证
			...
			if($rows['ord_amt'] == $total_fee) {
				...
				writelog('app.php?task=chongzhi&type=zyy_alipay&option=pay',json_encode(array('orderId'=>$out_trade_no,'ucid'=>$rowsp['ucid'],'payWay'=>$rows['cashier_code'],'amount'=>$total_fee,'orderStatus'=>'S','failedDesc'=>'','createTime'=>strtotime($notify_time) ,'status'=>0, 'oldYB'=>$rowsp['ingot'], 'newYB'=>$updateRole['ingot'])), $rowsp['player_level'], $rowsp['userid']);
			}
		}
    } else {
