...
if (empty($sessionid)) {
	...
	if ($mode == 1) {
		...
		$endpoint = "http://sp.sb.mobage-platform.cn/social/api/oauth/v2.01/request_token"; // Endpoint of token credential
	}
	...
	if ($mode == 1) {
		...
		$endpoint = "http://sp.sb.mobage-platform.cn/social/api/restful/v2/bank/debit/@app"; // Endpoint of bank transaction
	}
	...
	if ($mode == 1) {
		...
		$endpoint = "http://sp.sb.mobage-platform.cn/social/api/restful/v2/bank/debit/@app/$orderId?fields=state";
	}
	...
	if (isset($openInfo['Error'])) {
		$returnValue['status'] = 1030;
		$returnValue['message'] = '您的M币不足，请去梦宝谷中心充值！';
		echo json_encode($returnValue);
		exit;			
	}	
	$response = $putString = $putData = '';
