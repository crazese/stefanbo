...
if (!empty($_REQUEST)) {
	...
	//$insert['processed'] = 1;
	$insert['sysTime'] = time();
	$makesign = md5($insert['AppId'].$insert['Act'].$insert['ProductName'].$insert['ConsumeStreamId'].$insert['CooOrderSerial'].$insert['Uin'].$insert['GoodsId'].$insert['GoodsInfo'].$insert['GoodsCount'].$insert['OriginalMoney'].$insert['OrderMoney'].$insert['Note'].$insert['PayStatus'].$insert['CreateTime'].$_SC['appKey']);
	...
	if ($fwqdm != $_SC['fwqdm']) {
		if ($fwqdm == 'zyy02') {
			zfsj('http://sop2.jofgame.com/ucpay/91pay.php',$_REQUEST);
			exit;
		} elseif ($fwqdm == 'zjsh_zyy_001') {
			zfsj('http://zj01.jofgame.com/ucpay/91pay.php',$_REQUEST);
			exit;
		} elseif ($fwqdm == 'zyy03') {
			zfsj('http://sop3.jofgame.com/ucpay/91pay.php',$_REQUEST);
			exit;
		}
	}
	if ($insert['AppId'] != $_SC['AppId']) {
    ...
    if ($makesign == $sign) {
    	...
    	if ($insert['PayStatus'] == 1) {
	    	...
	    	$sql_user = "SELECT a.userid,b.playersid,b.ingot FROM ".$common->tname('user')." a,".$common->tname('player')." b WHERE a.username = '".$insert['Uin']."' && a.userid = b.userid LIMIT 1";
	    	...
	    	if (!empty($rows_user)) {
	    		...
	    		$db->query($sql_p);    			
	    		vipChongzhi($playersid, $insert['GoodsCount'], $yb, $insert['orderId']);
	    	}
