...
$app_key = '772d76d1f10e267e248bf9fac112eae5';
...
if ($gameid != $_SC['fwqdm']) {
	if ($gameid == 'zyy02') {
		zfsj('http://sop2.jofgame.com/ucpay/91djpay.php',$_REQUEST);
		exit;
	} elseif ($gameid == 'zjsh_zyy_001') {
		zfsj('http://zj01.jofgame.com/ucpay/91djpay.php',$_REQUEST);
		exit;
	} elseif ($gameid == 'zyy03') {
		zfsj('http://sop3.jofgame.com/ucpay/91djpay.php',$_REQUEST);
		exit;
	}
}
...
if ($mksign == $Sign) {
    ...
    writelog('app.php?task=chongzhi&type=91dj&option=pay',json_encode(array('orderId'=>$Urecharge_Id,'ucid'=>$rows_user['userid'],'payWay'=>'91dj','amount'=>$Recharge_Money,'orderStatus'=>'91dj','failedDesc'=>'','createTime'=>time(),'status'=>0,'newYB'=>$newYB,'oldYB'=>$oldYB)),$rows_user['player_level'],$rows_user['userid']);	    	
}
echo json_encode($backdata);