...
class Install(object):
	...
	def nginx_install(self):
		...
		conf = {
				'tar_path' 		: 	'/usr/local/lnmp/tar_package/nginx',
				'ftp_path' 		: 	'/lnmp/nginx',
				'init_path'		: 	'/usr/local/lnmp/app/init',
				'soft_path'		: 	'/usr/local/lnmp/app/nginx',
				'soft_name'		: 	'nginx',
				'user_name'		: 	'www-data'
				}
	...
	def mysql_install(self):
		...
		# install procedure setting
		conf = {
				'tar_path' 		: 	'/usr/local/lnmp/tar_package/mysql',
				'ftp_path' 		: 	'/lnmp/mysql',
				'init_path'		: 	'/usr/local/lnmp/app/init',
				'soft_path'		: 	'/usr/local/lnmp/app/mysql',
				'soft_name'		: 	'mysql',
				'user_name'		: 	'mysql',
				'mysql_user'	: 	'root',
				'mysql_pw'		:	'123456'
	...
	def php_install(self):
		...
		# install procedure setting
		conf = {
				'tar_path'		:		'/usr/local/lnmp/tar_package/php',
				'ftp_path'		:		'/lnmp/php',
				'init_path'		:		'/usr/local/lnmp/app/init',
				'soft_path'		:		'/usr/local/lnmp/app/php',
				'soft_name'		:		'php',
				'user_name'		:		'www-data',
				'mysql_path'	:		'/usr/local/lnmp/app/mysql',
				'php_config'	:		'/usr/local/lnmp/app/php/bin/php-config',
				'phpize_path'	:		'/usr/local/lnmp/app/php/bin/phpize'
		}
		...
		# export PATH again
		self.export_tool(conf['init_path'])
		self.export_tool('/usr/local/lnmp/app/mysql')
		# install php soft
		self.soft_install(conf['soft_name'],
						  soft_package,
						  conf['tar_path'],
	...
	def memcache_install(self):
		...
		conf = {
				'tar_path'		:		'/usr/local/lnmp/tar_package/memcache',
				'ftp_path'		:		'/lnmp/memcache',
				'init_path'		:		'/usr/local/lnmp/app/init',
				'soft_path'		:		'/usr/local/lnmp/app/memcache',
				'soft_name'		:		'memcache'
		}
		...
		print '''######## to start memcache, you need to run follow scripts in root:
				/usr/local/lnmp/app/memcache/bin/memcached -m 64 -p 11211 -u www-data -l 127.0.0.1'''
	...
	def init_pak_install(self, soft_list, tar_path, init_path):
		...
		for soft in soft_list:
			...
			if 'jpegsrc' in folder_name:
				...
				os.environ['PATH'] = '/usr/local/lnmp/sbin:/usr/local/lnmp/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/games'
				temp_options = '''--with-png=%s --with-freetype=%s --with-jpeg=%s''' % (init_path, init_path, init_path)
				file_config(os.path.join(tar_path, folder_name), init_path, temp_options)
				
				#os.environ['PATH'] = '/usr/local/lnmp/sbin:/usr/local/lnmp/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/games'

				# change 'zlib.h' to /usr/local/lnmp/app/init/include/zlib.h
	...
	def soft_install(self, name, soft_package, tar_path, soft_path, options):
		...
		if name == 'nginx':
			...
			result_1 = re.sub(r'./configure','./configure --prefix=/usr/local/lnmp/app/init',makefile_content)
			result = re.sub(r'./configure --prefix=/usr/local/lnmp/app/init --disable-shared','./configure --prefix=/usr/local/lnmp/app/init ', result_1)
	...
	def export_tool(self, path):
		'''
		Export the LD_LIBRARY_PATH to 'libpath' example like : /usr/local/lnmp/app/init/
		It will change to the directory /usr/local/lnmp/app/init/lib
		'''
