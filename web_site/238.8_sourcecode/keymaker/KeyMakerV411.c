	...
	for (x=1, y=0; x<argc; ++x) {
		if (x==1 && (parsedargs[x][0]=='/' || parsedargs[x][0]=='-')) {
			...
			if (y==0) strcpy(username, parsedargs[x]);
				...
				// strcpy(privatekey, parsedargs[x]); // remed by lili
				// strcpy(privatekey, "{8107F840-40AA-499f-9181-6DB19B9E27FA}");	// For Quicker Site License Version, Korean
				// strcpy(privatekey, "{A6A38AB2-3B29-4f61-AA0E-BE8EDE67EB51}");	// For Quicker Site License Version, English
				strcpy(privatekey, "{C8745082-8548-49e9-AF72-B43550827B06}");	// For Quicker Site License Version, CHT
			}
			else if (y==2) {
