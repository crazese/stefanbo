...
if ($makesign == $sign) {
		...
		if ($gameid != $_SC['fwqdm']) {
			if ($gameid == 'zyy02') {
				zfsj('http://sop1.jofgame.com/pay/ldpay.php',$_REQUEST);
				exit;
			} elseif ($gameid == 'zjsh_zyy_001') {
				zfsj('http://zj01.jofgame.com/pay/ldpay.php',$_REQUEST);
				exit;
			} elseif ($gameid == 'zyy03') {
				zfsj('http://zj03.jofgame.com/pay/ldpay.php',$_REQUEST);
				exit;
			} elseif ($gameid == 'zj3') {
				zfsj('http://117.135.139.237:8080/pay/ldpay.php',$_REQUEST);
				exit;
			}
	...
	//if ($insert['extraInfo'] == $_SC['fwqdm']) {
	    ...
	    writelog('app.php?task=chongzhi&type=ld&option=pay',json_encode(array('orderId'=>$insert['orderId'],'ucid'=>$insert['openid'],'payWay'=>'ldcz','amount'=>$insert['amount'],'orderStatus'=>$sfcg,'failedDesc'=>$insert['msg'],'createTime'=>$nowTime,'status'=>0,'newYB'=>$newYB,'oldYB'=>$oldYB)),$rows_user['player_level'],$rows_user['userid']);	   	
	//}
} else {	
