...
def main():
	...
	try:
		for arg in sys.argv:
			...
			elif arg.logwer() == '-u' or arg.lower() == '--usernamelist':
		...
		print R+"[-]Cheak your parameters input\n"+END
		help()
	...
	for u in user:
		for p in pwd:
			...
			if result != 1:
				print G+"[+]Attempt username: %s password: %s ..." % (u, p) + R + "Disable" + END
			else:
				print G+"[+]Attempt username: %s password: %s ..." % (u, p) + Y + "Enable" + END
			if not result:
				print R+"\n[-]There is no username and password enabled in the list."
				print "[-] Exiting... \n" + END
