...
$stat = $mc->add(MC.'CLI_ZYD_ZLZ',$memvalue, 0, 0);  //永不过期
if($stat === false)  {
  error_log(date("Y.m.d H:i:s")." zyd_zlz.php: memcache add error,annother process is running?\n");
...
$t = microtime(true);
//1小时或mysql,mc出错则终止循环
while (microtime(true) - $t < 1800 && mysql_errno() == 0 && $mc->get(MC.'CLI_ZYD_ZLZ')  === $memvalue) {
  //时间
  $mtime = explode(' ', microtime());
  $_SGLOBAL['timestamp'] = $mtime[1];
  $_SGLOBAL['supe_starttime'] = $_SGLOBAL['timestamp'] + $mtime[0];
...
$db->close();
if($mc->get(MC.'CLI_ZYD_ZLZ') === $memvalue) $normalrestart = true;
?>